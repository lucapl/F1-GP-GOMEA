...
run_job() {
    ...
    (cd "$full_output" && python3.12 "$cwd/run_ea_simple.py" \
	-opt "COGpath" \
	-generations 1000 \
	-popsize "$popsize" \
	-hof_size 1 \
	-path "$cwd/Framsticks52" \
	-sim "eval-allcriteria.sim;eval-once.sim;recording-body-coords.sim" \
	> "$logfile" 2>&1)
}
