...
from src.gomea import eaGOMEASubpop, forced_improvement, gom, override_nodes
...
def prepare_gomea_parser(parser):
    parser.add_argument("-p", "--popsize", default=50, type=int)
    ...
    parser.add_argument(
        ...
        type=int,
        default=[20, 30],
        help="Initial genotypes parts range",
    )
    parser.add_argument(
        "--neurons",
        nargs=2,
        type=int,
        default=[6, 8],
        help="Initial genotypes neurons range",
    )
    ...
    parser.add_argument(
        "--pmut", help="Probability of mutation occuring", default=0.8, type=float
    )
    parser.add_argument(
        "--fmut", help="Frequency of mutation occuring", default=10, type=int
    )
    parser.add_argument(
        "-t",
        "--test_function",
        default=3,
        choices=[3, 4, 5],
        help="Which test function to evaluate",
    )
    parser.add_argument(
        "--criteria", default="COGpath", help="Name of the evaluation function criteria"
    )
    parser.add_argument(
        "--subpops",
        default=20,
        type=int,
        help="Number of subpopulations to use in GOMEA",
    )
...
def generate_random(
    flib, parts: tuple[int, int], neurons: tuple[int, int], iters: int, geno_format="1"
):
    return flib.getRandomGenotype(
        flib.getSimplest(geno_format),
        *parts,
        *neurons,
        iters,
...
def create_ind(
    ...
    return parse(
        generate_random(flib, parts, neurons, iters, geno_format).replace(" ", ""), pset
    )
...
def main():
    ...
    toolbox.register(
        ...
        flib=framsLib,
        pset=pset,
        iters=args.initial_geno_mutations,
        parts=args.parts,
        neurons=args.neurons,
    )
    toolbox.register(
        "individual", tools.initIterate, creator.Individual, toolbox.random_individual
    )
    toolbox.register(
        "population", tools.initRepeat, list, toolbox.individual, args.popsize
    )
    ...
    toolbox.register(
        "genepool_optimal_mixing", gom, toolbox=toolbox, forcedImprov=isForcedImprov
    )
    toolbox.register("forced_improvement", forced_improvement, toolbox=toolbox)
    toolbox.register(
        "build_linkage_model", LinkageTreeFramsF1, original_control_word=None
    )
    toolbox.register("override_nodes", override_nodes, fillvalue="_", toolbox=toolbox)
    toolbox.register(
        "mutate", mutate, pset=pset, pmut=args.pmut, toolbox=toolbox, framsLib=framsLib
    )
    ...
    ########################
    try:
        new_pops, logbook = eaGOMEASubpop(
            populations,
            toolbox,
            logbook=logbook,
            fmut=args.fmut,
