...
class DensityDistribution:
    ...
    libm = find_library('m')  # for disabling/enabling floating point exceptions (division by zero occurs in the pyemd library)
    if libm is not None:  # libm.so (the mathematical library) is a part of Linux ecosystem - always present
        libm = cdll.LoadLibrary(libm)
    else:
    ...
    def normalize(self, signature):
        """Normalizes the signature values by dividing each element by the sum of all elements.
        Args:
            signature np.array(,dtype=float): A one-dimensional array of signature values.

        Returns:
            np.array(,dtype=float): A one-dimensional array of normalized signature values.
        """
        total = np.sum(signature)
        return np.divide(signature, total)
    ...
    def calculateDissimforVoxels(self, voxels1, voxels2):
        ...
        if self.frequency:
           s1 = abs(np.fft.fft(s1))
           s2 = abs(np.fft.fft(s2))
        ...
        if self.metric == 'l1':
            ...
            if self.frequency:
                num_points = np.linspace(0, 1, len(s1), True)
                dist_matrix = self.calculateDistanceMatrix(num_points,num_points)
            else:
                dist_matrix = self.calculateDistanceMatrix(s1[0],s2[0])

            if self.libm is not None:
                self.libm.fedisableexcept(0x04)  # change default flag value - don't cause "Floating point exception" when dividing by 0 (pyemd does that, for example when comparing two identical histograms, i.e., two identical signatures, for example from two identical phenotypes)

            if self.frequency:
                out = emd(self.normalize(s1),self.normalize(s2),np.array(dist_matrix,dtype=np.float64))
            else:
                out = emd(self.normalize(s1[1]),self.normalize(s2[1]),dist_matrix)
